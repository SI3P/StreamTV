...
5. confidence(v) = <img src="https://latex.codecogs.com/gif.latex?\frac{y(x(v))))}{\sum_{i=0}^{i<18}y(i*20))}-\frac{300}{y(x(v))))}" title="\frac{y(x(v))))}{\sum_{i=0}^{i<18}y(i*20))}-\frac{300}{y(x(v))))}" />
